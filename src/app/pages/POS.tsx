...
import { ShoppingCart, Plus, Minus, Trash2, Receipt, Printer } from 'lucide-react';
...
export default function POS() {
  ...
  const [purchaseMethod, setPurchaseMethod] = useState<'online' | 'offline'>('offline');
  ...
  const platformCommission = purchaseMethod === 'online'
    ? (onlinePlatform === 'shopee' ? 0.10 : 0.15)
    : 0;
  ...
  const printThermalReceipt = async (transactionData: any) => {
    ...
    try {
      ...
      if (!characteristic) throw new Error("Karakteristik Bluetooth tidak ditemukan");
      ...
      toast.error("Gagal terhubung ke printer thermal. Mencetak via browser...");
      window.print();
    } finally {
      setIsPrinting(false);
    }
  ...
  const handleCheckout = () => {
    ...
    const purchaseMethodLabel = purchaseMethod === 'online'
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-3xl font-semibold text-gray-900">Point of Sales (POS)</h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white rounded-lg border border-gray-200 p-6">
                <h2 className="font-semibold text-gray-900 mb-4">Pilih Produk</h2>

                <div className="mb-4">
                  <h3 className="text-sm font-medium text-gray-700 mb-3">Paradose - Coffee</h3>
                  <div className="grid grid-cols-2 gap-4">
                    {products.filter(p => p.brand === 'Paradose').map(product => (
                      <button
                        key={product.id}
                      ...
                      >
                        ...
                        {!isProductAvailable(product.id) && (
                          <div className="text-xs text-red-500 mt-1 font-bold">Stok Habis</div>
                        )}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  <h3 className="text-sm font-medium text-gray-700 mb-3">Parasoes - Choux Pastry</h3>
                  <div className="grid grid-cols-2 gap-4">
                    {products.filter(p => p.brand === 'Parasoes').map(product => (
                      <button
                        key={product.id}
                        onClick={() => addToCart(product)}
                        disabled={!isProductAvailable(product.id)}
                ...
                <div className="border-t pt-4 space-y-2 mb-4">
                  ...
                  {commission > 0 && (
                    <div className="flex justify-between text-red-600">
                      <span>Komisi Platform ({(platformCommission * 100).toFixed(0)}%):</span>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mb-2">
                      <button
                        ...
                        className={`flex-1 px-4 py-2 rounded-lg font-medium transition-colors ${purchaseMethod === 'offline'
                      ...
                      <button
                        ...
                        className={`flex-1 px-4 py-2 rounded-lg font-medium transition-colors ${purchaseMethod === 'online'
                            ? 'bg-blue-600 text-white'
                            : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                          }`}
                      >
